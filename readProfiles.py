...
def read_replicate_level_profiles(dataset_rootDir,dataset,profileType,per_plate_normalized_flag):
    ...
    dataDir = dataset_rootDir+'/'+ds_info_dict[dataset][0]
...
def read_treatment_level_profiles(dataset_rootDir,dataset,profileType,filter_repCorr_params,per_plate_normalized_flag):
    ...
    elif filter_perts == 'highRepUnion_and_negcon':
        highRepPerts = highRepFinder(
            dataset, 'union', repCorrFilePath) + ['negcon','DMSO']

        cp_data_repLevel = cp_data_repLevel[cp_data_repLevel['PERT'].isin(
            highRepPerts)].reset_index()
        l1k_data_repLevel = l1k_data_repLevel[l1k_data_repLevel['PERT'].isin(
            highRepPerts)].reset_index()
